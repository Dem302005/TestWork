...
public class TimelinePlayableWizard : EditorWindow
{
    ...
    */
    ...
    private readonly GUIContent m_BehaviourVariablesContent = new GUIContent("Behaviour Variables",
    ...
    private readonly GUIContent m_CCAllContent =
        new GUIContent("All", "Your playable supports all of the above features.");

    private readonly GUIContent m_CCBlendingContent =
        new GUIContent("Blending", "Your playable supports overlapping of clips to blend between them.");

    private readonly GUIContent m_CCClipInContent =
        new GUIContent("Clip In", "Your playable need not be at the start of the Timeline.");

    private readonly GUIContent m_CCExtrapolationContent = new GUIContent("Extrapolation",
        "Your playable will persist beyond its end time and its results will continue until the next clip is encountered.");

    private readonly GUIContent m_CCLoopingContent = new GUIContent("Looping",
    ...
    private readonly GUIContent m_CCNoneContent =
        new GUIContent("None", "Your playable supports none of the features below.");

    private readonly GUIContent m_CCSpeedMultiplierContent =
        new GUIContent("Speed Multiplier", "Your playable supports changes to the time scale.");

    private readonly GUIContent m_ClipCapsContent = new GUIContent("Clip Caps",
    ...
    private readonly GUIContent m_CreateDrawerContent = new GUIContent("Create Drawer?",
    ...
    private readonly GUIContent m_DefaultValuesComponentContent = new GUIContent("Default Values",
    ...
    private readonly GUIContent m_ExposedReferencesContent = new GUIContent("Exposed References",
    ...
    private readonly GUIContent m_PlayableNameContent = new GUIContent("Playable Name",
    ...
    private readonly GUIContent m_ShowHelpBoxesContent =
        new GUIContent("Show Help", "Do you want to see the help boxes as part of this wizard?");

    private readonly GUIContent m_StandardBlendPlayableContent = new GUIContent("Standard Blend Playable",
    ...
    private readonly GUIContent m_StandardBlendPlayablePropertiesContent = new GUIContent(
    ...
    private readonly GUIContent m_TrackBindingTypeContent = new GUIContent("Track Binding Type",
        "This is the type of object the Playable will affect.  E.G. To affect the position choose Transform.");

    private readonly GUIContent m_TrackColorContent = new GUIContent("Track Color",
    ...
    private UsableProperty[] m_UsableProperties;
    ...
    private void OnGUI()
    {
        ...
        if (showHelpBoxes)
        {
            EditorGUILayout.HelpBox("This wizard is used to create the basics of a custom playable for the Timeline. "
                                    ...
                                    + "with playables and the Timeline, you may which to create your own scripts instead.",
                MessageType.None);
            EditorGUILayout.Space();
        }
        ...
        if (isStandardBlendPlayable)
        {
            ...
            defaultValuesComponent = EditorGUILayout.ObjectField(m_DefaultValuesComponentContent,
                defaultValuesComponent, trackBinding.type, true) as Component;
        }
        else
        {
        ...
        if (isStandardBlendPlayable)
        {
            StandardBlendPlayablePropertyGUI(oldIndex != m_ComponentBindingTypeIndex ||
                                             oldStandardBlendPlayable != isStandardBlendPlayable);
        }
        else
        {
            exposedVariablesNamesValid = VariableListGUI(exposedReferences, s_ExposedReferenceTypes,
                m_ExposedReferencesContent, "newExposedReference");

            EditorGUILayout.Space();
            EditorGUILayout.Space();

            scriptVariablesNamesValid = VariableListGUI(playableBehaviourVariables, s_BehaviourVariableTypes,
                m_BehaviourVariablesContent, "newBehaviourVariable");
        ...
        if (playableNameNotEmpty && playableNameFormatted && allUniqueVariableNames && exposedVariablesNamesValid &&
            scriptVariablesNamesValid && !playableNameTooLong)
        ...
        if (m_CreateButtonPressed)
            switch (m_CreationError)
            {
                ...
                case CreationError.PlayableDrawerAlreadyExists:
                    EditorGUILayout.HelpBox(
                        "The type " + playableName + k_PropertyDrawerSuffix + " already exists, no files were created.",
                        MessageType.Error);
                    break;
            }
    ...
    {
        var componentTypes = typeof(Component).Assembly.GetTypes().Where(t => typeof(Component).IsAssignableFrom(t))
            .ToArray();
        var componentUsableTypes = UsableType.GetUsableTypeArray(componentTypes);
        ...
        var uiTypes = typeof(Text).Assembly.GetTypes().Where(t => typeof(Component).IsAssignableFrom(t)).ToArray();
        var uiUsableTypes = UsableType.GetUsableTypeArray(uiTypes);

        var componentTypesList = UsableType.AmalgamateUsableTypes(componentUsableTypes, uiUsableTypes).ToList();
        componentTypesList.Sort();
        s_ComponentTypes = componentTypesList.ToArray();

        var exposedRefTypeList = UsableType.AmalgamateUsableTypes(defaultUsableTypes, uiUsableTypes).ToList();
    ...
    private void StandardBlendPlayablePropertyGUI(bool findNewProperties)
    {
        ...
        {
            IEnumerable<PropertyInfo> propertyInfos = trackBinding.type.GetProperties(BindingFlags.Instance |
                BindingFlags.Public | BindingFlags.SetProperty | BindingFlags.GetProperty);
            propertyInfos = propertyInfos
                .Where(x => IsTypeBlendable(x.PropertyType) || IsTypeAssignable(x.PropertyType)).OrderBy(x => x.Name);
    ...
    private bool VariableListGUI(List<Variable> variables, UsableType[] usableTypes, GUIContent guiContent,
        string newName)
    {
        EditorGUILayout.BeginVertical(GUI.skin.box);
    ...
    private CreationError CreateScripts()
    {
        ...
        if (isStandardBlendPlayable)
        {
            ...
            var path = Application.dataPath + "/" + playableName + "/Editor/" + playableName + k_PropertyDrawerSuffix +
                       ".cs";
        ...
        else
        {
            ...
            if (m_CreateDrawer)
            {
                ...
                var path = Application.dataPath + "/" + playableName + "/Editor/" + playableName +
                           k_PropertyDrawerSuffix + ".cs";
                using (var writer = File.CreateText(path))
                {
    ...
    private bool ScriptAlreadyExists(string scriptName)
    {
    ...
    private string PlayableBehaviourVariablesToString()
    {
        var returnVal = "";
        for (var i = 0; i < playableBehaviourVariables.Count; i++)
            returnVal += k_Tab + "public " + playableBehaviourVariables[i].usableType.name + " " +
    ...
    private string ScriptVariablesAsSerializedPropAssignmentToString()
    {
        var returnVal = "";
        for (var i = 0; i < playableBehaviourVariables.Count; i++)
            returnVal += k_Tab + k_Tab + "SerializedProperty " + playableBehaviourVariables[i].name +
                         "Prop = property.FindPropertyRelative(\"" + playableBehaviourVariables[i].name + "\");\n";
        return returnVal;
    }
    ...
    private string ScriptVariablesAsSerializedPropGUIToString()
    {
        ...
        for (var i = 0; i < playableBehaviourVariables.Count; i++)
        {
            returnVal += k_Tab + k_Tab + "EditorGUI.PropertyField(singleFieldRect, " +
                         playableBehaviourVariables[i].name + "Prop);\n";
    ...
    private string StandardBlendTrackAssetScript()
    {
        return
            ...
            "\n" +
            k_Tab + k_Tab + k_Tab + "driver.AddFromName<" + trackBinding.name +
            ">(trackBinding.gameObject, iterator.propertyPath);\n" +
            k_Tab + k_Tab + "}\n" +
            "#endif\n" +
    ...
    private string StandardBlendTrackBindingPropertiesDefaultsDeclarationToString()
    {
        var returnVal = "";
        for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
            returnVal += k_Tab + standardBlendPlayableProperties[i].type + " " +
                         standardBlendPlayableProperties[i].NameAsPrivateDefault + ";\n";
        return returnVal;
    }
    ...
    private string StandardBlendTrackBindingPropertiesDefaultsAssignmentToString()
    {
        var returnVal = "";
        for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
            returnVal += k_Tab + k_Tab + k_Tab + standardBlendPlayableProperties[i].NameAsPrivateDefault +
                         " = m_TrackBinding." + standardBlendPlayableProperties[i].name + ";\n";
        return returnVal;
    }
    ...
    private string StandardBlendBlendedVariablesWeightedIncrementationToString()
    {
        ...
        for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
            if (standardBlendPlayableProperties[i].usability == UsableProperty.Usability.Blendable)
                returnVal += k_Tab + k_Tab + k_Tab + standardBlendPlayableProperties[i].NameAsLocalBlended +
                             " += input." + standardBlendPlayableProperties[i].name + " * inputWeight;\n";
        return returnVal;
    }
    ...
    private string StandardBlendAssignableVariablesAssignedBasedOnGreatestWeightToString()
    {
        ...
        for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
            if (standardBlendPlayableProperties[i].usability == UsableProperty.Usability.Assignable)
                returnVal += k_Tab + k_Tab + k_Tab + k_Tab + "m_TrackBinding." +
                             standardBlendPlayableProperties[i].name + " = input." +
                             standardBlendPlayableProperties[i].name + ";\n";
    ...
    private string StandardBlendTrackBindingPropertiesBlendedAssignmentToString()
    {
        ...
        for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
        {
            ...
            if (prop.type == "int")
                returnVal += k_Tab + k_Tab + "m_TrackBinding." + prop.name + " = Mathf.RoundToInt (" +
                             prop.NameAsLocalBlended + " + " + prop.NameAsPrivateDefault + " * (1f - totalWeight));\n";
            else
                returnVal += k_Tab + k_Tab + "m_TrackBinding." + prop.name + " = " + prop.NameAsLocalBlended + " + " +
    ...
    private string StandardBlendTrackBindingPropertiesAssignableAssignmentToString()
    {
        ...
        {
            var returnVal = k_Tab + k_Tab + "if (currentInputs != 1 && 1f - totalWeight > greatestWeight)\n";
            ...
            for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
            {
                ...
                returnVal += k_Tab + k_Tab + k_Tab + "m_TrackBinding." + standardBlendPlayableProperties[i].name +
                             " = " + standardBlendPlayableProperties[i].NameAsPrivateDefault + ";\n";
            }
    ...
    private string StandardBlendTrackBindingPropertiesDefaultAssignmentToString()
    {
        var returnVal = "";
        for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
            returnVal += k_Tab + k_Tab + "m_TrackBinding." + standardBlendPlayableProperties[i].name + " = " +
                         standardBlendPlayableProperties[i].NameAsPrivateDefault + ";\n";
        return returnVal;
    }
    ...
    private string StandardBlendTrackBindingPropertiesAsSerializedPropsDeclarationToString()
    {
        var returnVal = "";
        for (var i = 0; i < standardBlendPlayableProperties.Count; i++)
            returnVal += k_Tab + k_Tab + "SerializedProperty " +
                         standardBlendPlayableProperties[i].NameAsLocalSerializedProperty +
                         " = property.FindPropertyRelative(\"" + standardBlendPlayableProperties[i].name + "\");\n";
        return returnVal;
    }
    ...
    public class Variable : IComparable
    {
        ...
        public string NameAsPrivate
        {
            get
            {
                var returnVal = "m_" + name[0].ToString().ToUpper();
                for (var i = 1; i < name.Length; i++) returnVal += name[i];
                return returnVal;
            }
        }
    ...
    public class UsableProperty : IComparable
    {
        ...
        public void CreateSettingDefaultValueString(Component defaultValuesComponent)
        {
            ...
            var defaultValueObj = propertyInfo.GetValue(defaultValuesComponent, null);
            ...
            switch (type)
            {
                ...
                case "string":
                    ...
                    break;
                case "Quaternion":
                    var defaultQuaternionValue = (Quaternion)defaultValueObj;
                    defaultValue = "new Quaternion(" + defaultQuaternionValue.x + "f, " + defaultQuaternionValue.y +
                                   "f, " + defaultQuaternionValue.z + "f, " + defaultQuaternionValue.w + "f)";
                    break;
                case "bool":
        ...
        {
            var returnVal = new string[usableProperties.Length];
        ...
        {
            var usableProperties = new UsableProperty[propertyInfos.Length];
            for (var i = 0; i < usableProperties.Length; i++)
