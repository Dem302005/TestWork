...
public class AutoSetQualitySetting
{
    ...
    {
        if (!File.Exists(Application.dataPath + "/../Library/QualityDetected"))
        {
            File.WriteAllText(Application.dataPath + "/../Library/QualityDetected",
                "Delete this to trigger a new quality settingsdetection");
            DetectQualitySettings();
        }
    }

    [MenuItem("Kit Tools/Auto Detect Quality Settings")]
    private static void DetectQualitySettings()
    {
    ...
    {
        var force = QualitySettings.GetQualityLevel() < 1;
