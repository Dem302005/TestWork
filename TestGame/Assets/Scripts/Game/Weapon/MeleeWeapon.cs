    ...
    public class MeleeWeapon : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            if (m_InAttack)
                for (var i = 0; i < attackPoints.Length; ++i)
                {
                    ...
                    var attackVector = worldPos - m_PreviousPos[i];
                    ...
                    var contacts = Physics.SphereCastNonAlloc(r, pts.radius, s_RaycastHitCache, attackVector.magnitude,
                        ~0,
                        QueryTriggerInteraction.Ignore);

                    for (var k = 0; k < contacts; ++k)
                    {
                        var col = s_RaycastHitCache[k].collider;
        ...
        private void OnDrawGizmosSelected()
        {
            for (var i = 0; i < attackPoints.Length; ++i)
            {
                ...
                {
                    var worldPos = pts.attackRoot.TransformVector(pts.offset);
                    Gizmos.color = new Color(1.0f, 1.0f, 1.0f, 0.4f);
                    Gizmos.DrawSphere(pts.attackRoot.position + worldPos, pts.radius);
                }

                if (pts.previousPositions.Count > 1) Handles.DrawAAPolyLine(10, pts.previousPositions.ToArray());
            }
        }
...
#if UNITY_EDITOR
            for (var i = 0; i < attackPoints.Length; ++i) attackPoints[i].previousPositions.Clear();
#endif
        }
