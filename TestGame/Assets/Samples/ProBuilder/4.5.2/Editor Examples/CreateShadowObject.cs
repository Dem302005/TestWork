...
namespace ProBuilder.ExampleActions
{
    ...
    {
        private static readonly GUIContent k_VolumeSize = new GUIContent("Volume Size",
            "How far the shadow volume extends from " +
        ...
        public override ToolbarGroup group => ToolbarGroup.Object;
        public override Texture2D icon => null;
        public override TooltipContent tooltip => k_Tooltip;

        private static bool showPreview
        {
            get => EditorPrefs.GetBool("pb_shadowVolumePreview", true);
        ...
        public override bool enabled => MeshSelection.selectedObjectCount > 0;

        /// <summary>
        ///     Determines if the action should be loaded in the menu (ex, face actions shouldn't be shown when in vertex editing
        ///     mode).
        /// </summary>
        /// <returns></returns>
        ...
        protected override void OnSettingsGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            var volumeSize = EditorPrefs.GetFloat("pb_CreateShadowObject_volumeSize", .07f);
            ...
            EditorGUI.BeginChangeCheck();
            var shadowMode = (ShadowCastingMode)EditorPrefs.GetInt("pb_CreateShadowObject_shadowMode",
            ...
            EditorGUI.BeginChangeCheck();
            var extrudeMethod =
                (ExtrudeMethod)EditorPrefs.GetInt("pb_CreateShadowObject_extrudeMethod", (int)ExtrudeMethod.FaceNormal);
...
//				MenuOption.CloseAll();
        ...
        public override ActionResult DoAction()
        {
            var shadowMode = (ShadowCastingMode)EditorPrefs.GetInt("pb_CreateShadowObject_shadowMode",
                (int)ShadowCastingMode.ShadowsOnly);
            var extrudeDistance = EditorPrefs.GetFloat("pb_CreateShadowObject_volumeSize", .08f);
            var extrudeMethod =
                (ExtrudeMethod)EditorPrefs.GetInt("pb_CreateShadowObject_extrudeMethod", (int)ExtrudeMethod.FaceNormal);

            foreach (var mesh in MeshSelection.top)
            {
                var shadow = GetShadowObject(mesh);

                if (shadow == null)
                    continue;

                foreach (var f in shadow.faces)
                {
...
#if !UNITY_4_6 && !UNITY_4_7
                var mr = shadow.gameObject.GetComponent<MeshRenderer>();
        ...
        private ProBuilderMesh GetShadowObject(ProBuilderMesh mesh)
        {
            ...
            for (var i = 0; i < mesh.transform.childCount; i++)
            {
                var t = mesh.transform.GetChild(i);

                if (t.name.Equals(string.Format("{0}-ShadowVolume", mesh.name)))
                {
                    var shadow = t.GetComponent<ProBuilderMesh>();

                    if (shadow != null)
                    {
                        Undo.RecordObject(shadow, "Update Shadow Object");

                        var faces = new Face[mesh.faceCount];

                        for (var nn = 0; nn < mesh.faceCount; nn++)
            ...
            var newShadowMesh = ProBuilderMesh.Create();
