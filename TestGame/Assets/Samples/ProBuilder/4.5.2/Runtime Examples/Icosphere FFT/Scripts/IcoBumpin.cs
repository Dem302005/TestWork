...
namespace ProBuilder.Examples
{
    ...
    public class IcoBumpin : MonoBehaviour
    {
        ...
        private readonly float[][] m_FourierSamples = new float[2][]
        {
        ...
        private readonly float[][] m_RawSamples = new float[2][]
        {
        ...
        private readonly float[] m_Rms = new float[2];
        ...
        private void Update()
        {
            var currentFrame = m_FrameIndex;
            var previousFrame = (m_FrameIndex + 1) % 2;
            ...
            for (var i = 0; i < m_AnimatedSelections.Length; i++)
            {
                var normalizedIndex = i / m_FaceLength;

                var n = (int)(normalizedIndex * fftBounds);

                var displacement = m_AnimatedSelections[i].normal *
                ...
                foreach (var t in m_AnimatedSelections[i].indices)
            ...
            // Waveform ring
            for (var i = 0; i < k_WaveformSampleCount; i++)
            {
                var n = i < k_WaveformSampleCount - 1 ? i : 0;
                var travel = waveformRadius + (m_RawSamples[currentFrame][n] + m_RawSamples[previousFrame][n]) * .5f *
                    waveformHeight;
