...
namespace Gamekit3D.WorldBuilding
{
    ...
    public class MegaSmartObjectEditor : Editor
    {
        private BoxBoundsHandle bbh;
            ...
            using (new Handles.DrawingScope(Color.yellow, mso.transform.localToWorldMatrix))
            {
                ...
                using (var cc = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    if (cc.changed)
                    {
                        ...
                        var volume = max - min;
                        var steps = new Vector3Int(Mathf.FloorToInt(volume.x / size.x),
                            Mathf.FloorToInt(volume.y / size.y), Mathf.FloorToInt(volume.z / size.z));
